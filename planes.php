    ...
    <section id="planes" class="pricing-section">
        <article class="pricing-article">
            ...
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
                <input type="hidden" name="plan_nombre" value="plan basico"> <!-- Nombre del plan específico -->
                <label for="codigo">Código de selección:</label>
                <select name="codigo" id="codigo">
                <?php
                // Realizar consulta para obtener los códigos del plan básico
                $consultaplan = "SELECT codigo FROM planes WHERE nombre_plan = 'plan basico'";
                $resultadoplan = mysqli_query($conn, $consultaplan);
                // ... Ejecutar la consulta y obtener los resultados

                // Recorrer los resultados y generar las opciones del select
                while ($row = $resultadoplan->fetch_assoc()) {
                $codigo = $row['codigo'];
                // Generar la opción con el valor y texto del código
                echo "<option value='$codigo'>$codigo</option>";
                }
                ?>
                </select>
                <input type="text" name="tarjeta" id="tarjeta"  maxlength="16" required pattern="[0-9]{16}" placeholder="Ingrese el número de tarjeta (16 dígitos)">

                <button name="procesar_tarjeta" type="submit">Comprar</button>
            </form>
            <?php
            if (isset($_POST['procesar_tarjeta'])) {
                // Obtener el código seleccionado por el usuario desde el formulario
                $codigo = $_POST['codigo']; // Asegúrate de usar el nombre correcto del input en tu formulario
                            
                // Crear la consulta SQL para insertar en la tabla "control_planes"
                $sql_control_plan = "INSERT INTO control_planes (plan_usuario, plan_codigo) VALUES ('$usuario', '$codigo')";

                // Ejecutar la consulta
                if ($conn->query($sql_control_plan) === TRUE) {
                    // Borrar el plan de la tabla planes
                    $deleteQuery = "DELETE FROM planes WHERE codigo = '$codigo'";
                    mysqli_query($conn, $deleteQuery);

                    header("Location: suscripciones.php");
                    exit();
                } else {
                    echo "Error al insertar el registro en la tabla control_planes: " . $conn->error;
                }
            }
        ...
        <article class="pricing-article">
            ...
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
                <input type="hidden" name="plan_nombre" value="plan premium"> <!-- Nombre del plan específico -->
                <label for="codigo">Código de selección:</label>
                <select name="codigo" id="codigo">
                <?php
                // Realizar consulta para obtener los códigos del plan básico
                $consultaplan = "SELECT codigo FROM planes WHERE nombre_plan = 'plan premium'";
                $resultadoplan = mysqli_query($conn, $consultaplan);
                // ... Ejecutar la consulta y obtener los resultados
        ...
        <article class="pricing-article">
            ...
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
                <input type="hidden" name="plan_nombre" value="plan familiar"> <!-- Nombre del plan específico -->
                <label for="codigo">Código de selección:</label>
                <select name="codigo" id="codigo">
                <?php
                // Realizar consulta para obtener los códigos del plan básico
                $consultaplan = "SELECT codigo FROM planes WHERE nombre_plan = 'plan familiar'";
                $resultadoplan = mysqli_query($conn, $consultaplan);
                // ... Ejecutar la consulta y obtener los resultados
